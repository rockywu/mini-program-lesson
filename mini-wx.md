...
在我们了解微信小程序过程中，最重要的模块分为：`App`、`Page`、`Component`

分别定义：

* `App` 函数，用于创建一个应用，一个小程序只有只能调度一次App函数
* `Page` 函数，用于创建一个页面，同时需要配置模板、样式、配置文件一同使用
* `Component` 函数，用于创建一个自定义组件，同时需要配置模板、样式、配置文件一同使用
...
微信小程序提供的自定义组件，方便的提供了开发者模块化、组件化开发业务模块和场景支持。
简单列出4个微信小程序自定义组件分为：[更多详细介绍](https://developers.weixin.qq.com/miniprogram/dev/framework/custom-component/wxml-wxss.html)
...
组件的每个生命周期都可以在特定的场景中使用，请按照特定场景需求进行合理安排。
